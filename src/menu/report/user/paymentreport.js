...
let addition=(a,b)=>{
  ...
  let getTopic = async () => {
    ...
    let response = await api.get('get_payment_api_view_user/')
    ...
    if (response.data.success === 1) {
      ...
      // for (let i = 0; i < sample.length; i++) {
        ...
        let options = {
            machine_id:a.m_id,// sample[i].Machine_Id,
           // payment_date: sample[i].refill_date,
            ...
            total_amount:parseInt(a.coin_mode_counts,10)+parseInt(a.qr_mode_counts,10)
             + '/' +
  ...
  const exportReport_FromTOdate = async (e) => {
    ...
    // let response = await api.post('machine/user_payment_report/', {
    //   from_date: formatDate1(twodate.split('=')[0]),
    //   to_date: formatDate1(twodate.split('=')[1]),
    // })
    let response=await api.get(`machine/user_payment_report/?from_date=${formatDate1(twodate.split('=')[0])}&to_date=${formatDate1(twodate.split('=')[1])}`)
...
function downloadExcel() {
    ...
    // Example data
    const headers = [["Sr.No.", "MACHINE ID", "COIN COUNT","QR COUNT","TOTAL AMOUNT"]];
    ...
    XLSX.writeFile(workbook, "VendingMAchineUserPaymentReport.xlsx");
  
}
function excel_url(){
  const url = `${baseURL}/machine/user_payment_report_excel_download/?from_date=${formatDate1(startDate)}&to_date=${formatDate1(endDate)}`;
  //  return url;
