...
function Topic() {
  ...
  let getTopic = async () => {
    ...
    let response = await api.get('get_payment_api_view/')
    ...
    if (response.data.success === 1) {
      ...
      for (let i = 0; i < response.data.result.length; i++) {
        ...
        let options = {
          ...
          payment_date: formatDate(a.created_at),
          coin_count: a.coin_mode_counts + '/' + parseInt(a.coin_mode_counts) * 10,
          qr_pay_count: a.qr_mode_counts + '/' + parseInt(a.qr_mode_counts) * 10,
          total_amount:
  ...
  const exportReport_FromTOdate = async (e) => {
    ...
    let response = await api.post('post_payment_report_excel/', {
      // from: startDate,
      // to: endDate,
    ...
    if(response.data.result.length<=0)
      { setNotes([]);
        alert("there is no data in provided date range");
       return;
      }
  ...
  function downloadExcel() {
    ...
    //     const address = XLSX.utils.encode_col(C) + "1"; // Construct address of each cell in the first row
    //     if(!worksheet[address]) continue; // Check if cell exists
    //     worksheet[address].s = headerStyle; // Apply style
    // }
    ...
    var filename = 'payment_report' + formatDate(new Date()) + '.xlsx'
    // Generate an XLSX file and trigger the download
    XLSX.writeFile(workbook, filename)
  }

  function excel_url() {
    const url = `${baseURL}/get_payment_report_excel_download/?from_date=${formatDate1(
      startDate,
  ...
  function pdfdownload() {
    ...
    tabledata =
      ...
      '<div class="justify-content: center"><table border="1" style="margin-left: auto;margin-right: auto;border-bottom: 1px solid #ddd; border-collapse: collapse; font-size: 14px;text-align: center; align: center; border-color: #96D4D4;">' +
              ...
              {/* <CCard className="mb-1 shadow bg-body rounded">
                <CCardBody> */}
                  <CSmartTable
                    ...
                    itemsPerPage={10}
                    pagination
                    //  footer={['','','','Total : '+coinCount+'/'+coinAmount ,'Total : '+qrCount+'/'+qrAmount,'Total : '+totaltransaction+'/'+totalAmount]}
                    // onRowClick={}
                    scopedColumns={{
                      Action: (item) => (
                        <td>
                          <Link
                            to={'/superadmin/user/paymentdetail/report/' + `${item.machine_id}`}
                          >
