...
const Complaint = () => {
  ...
  let getTopic = async () => {
    let response = await api.get('saas/saasapp/clientfeedback/')
  ...
  const saveTopic = async (e) => {
    ...
    if (form.checkValidity() === true) {
          ...
          let response = await api.post('/saas/saasapp/clientfeedback/', uploadData);
         
        if (response.data.success === 1) {
          alert('Client Feedback  Submitted .');
          setValidated(false);
          setVisible1(false);
          getTopic();
          getrefresh();
    ...
    <CCard className="h-auto" style={{ boxShadow: '0px 4px 4px rgba(0, 0, 0, 0.25)', borderRadius: '10px' }}>
      <CForm
        ...
        <CRow className="justify-content-center mx-4">
          <b className='justify-content-center mx-4'>Client Feedback</b>
