...
function Topic() {
    ...
    const saveTopic = async (e) => {
        ...
        if (form.checkValidity() === true) {
            if (notes1?.includes(e.target.machine_id.value)) {
                ...
                let response = await api.post('/adminapp/machine-masters/', uploadData)
                if (response.data.success === 1) {
    ...
    const updateTopic = async (e) => {
        ...
        if (form.checkValidity() === true) {
            const uploadData = new FormData(e.target)
            let response = await api.patch(`/adminapp/machine-masters/${id}/`, uploadData)
            if (response.data.success === 1) {
    ...
    const deleteTopic = async (id) => {
        let response = await api.delete(`/adminapp/machine-masters/${id}/`)
        if (response.data.success === 1) {
            // alert(response.data.message)
