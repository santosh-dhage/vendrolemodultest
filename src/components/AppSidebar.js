...
//         dispatch({ type: 'set', sidebarShow: visible })
//       }}
//       style={{ backgroundColor: selectedColor }} // Apply selectedColor as the background color
//     >
//       <CSidebarBrand className="d-none d-md-flex" to="/" style={{ backgroundColor: selectedColor }}>
//         <CHeaderNav>
//           <CRow sm={12}>
...
//       {selectedColor && ( // Only render the logout button if selectedColor is not null
//         <div style={{ padding: '10px 14px', backgroundColor: selectedColor }}>
//           <NavLink
//             to="/login"
//             style={{ color: 'white', textDecoration: 'none' }}
...
const AppSidebar = () => {
  ...
  const getTopic = async () => {
    ...
    if (authTokens.user.role !== '1' && authTokens.user.role !== '2') {
      const response1 = await api.get('machine/users/module/');
      if (response1.data.success === 1) {
        let a = response1.data.result;
        let ddd = [];
  ...
  if (authTokens.user.role !== '1' && authTokens.user.role !== '2') {
    ...
    if (modules_no.includes("Machine Master")) {
      ...
      custommodul.push(machineMaster)
    }
    ...
    if (modules_no.includes("Machine Mapping")) {
      ...
      custommodul.push(machineMapping)
    }
